...
func TestPubSub_single_slow_consumer(t *testing.T) {
	...
	bus.Publish(topic, 1)
	<-slow.started // wait consumer to receive 1st message
	bus.Publish(topic, 2)
	bus.Publish(topic, 3)
	bus.Publish(topic, 4)
	bus.Publish(topic, 5)
...
func TestPubSub_two_consumers_slow_and_fast(t *testing.T) {
	...
	slow := testConsumer{}
	slow.subscribe(topic, &bus)
	defer func() { slow.done <- true }()

	normal := testConsumer{}
	normal.subscribe(topic, &bus)
	defer func() { normal.done <- true }()

	// normal consumer should consume 1, 2, 3, 4 & 5
	bus.Publish(topic, 1)

	<-slow.started   // wait consumer to receive 1st message
	<-normal.started // wait consumer to receive 1st message
...
func TestPubSub_late_subscription_gets_latest_value(t *testing.T) {
	...
	// When I subscribe to the topic after the publish event
	normal := testConsumer{}
	normal.subscribe(topic, &bus)
	defer func() { normal.done <- true }()
