...
func (job *UploadJobT) Aborted(attempts int, startTime time.Time) bool {
	...
	toRetry := attempts > minRetryAttempts && timeutil.Now().Sub(startTime) > retryTimeWindow
	pkgLogger.Errorf("attempts: %d, minRetryAttempts: %d, startTime: %v, retryTimeWindow: %d, toRetry: %t", attempts, minRetryAttempts, startTime, retryTimeWindow/time.Minute, toRetry)

	// Given we have reached the attempts and the retry window has expired as well.
